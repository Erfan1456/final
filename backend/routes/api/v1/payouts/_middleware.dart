...
Handler middleware(Handler handler) {
  return (context) async {
    try {
      final service = _tryRead<PayoutWebhookService>(context);
      if (service != null) {
        return await handler(
          context.provide<PayoutWebhookService>(() => service),
        );
      }
      final mongo = context.read<MongoDatabase>();
      final config = context.read<ServerConfig>();
      final webhooks = await RoleScopedComposition.payoutWebhooks(
        mongo: mongo,
        config: config,
      );
      return await handler(
        context.provide<PayoutWebhookService>(() => webhooks),
      );
